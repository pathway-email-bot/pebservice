...
class TestGetProjectId:
    ...
    def setup_method(self):
        ...
        from service.secret_utils import _reset_caches
        _reset_caches()
    ...
    def test_reads_from_gcp_project_env(self):
        with patch.dict("os.environ", {"GCP_PROJECT": "my-project"}, clear=False):
            from service.secret_utils import get_project_id
    ...
    def test_reads_from_google_cloud_project_env(self):
        with patch.dict(
            ...
            from service.secret_utils import get_project_id, _reset_caches
            _reset_caches()
...
class TestGetSecret:
    ...
    def test_fetches_and_strips_secret(self):
        with patch.dict("os.environ", {"GCP_PROJECT": "test-proj"}, clear=False):
            with patch("service.secret_utils.secretmanager") as mock_sm:
                ...
                from service.secret_utils import get_secret, _reset_caches
                _reset_caches()
...
class TestGetOpenaiApiKey:
    ...
    def test_returns_key_from_secret_manager(self):
        with patch("service.secret_utils.get_secret", return_value="sk-test-key"):
            from service.secret_utils import get_openai_api_key
            assert get_openai_api_key() == "sk-test-key"

    def test_falls_back_to_env_var(self):
        with patch("service.secret_utils.get_secret", side_effect=Exception("no SM")):
