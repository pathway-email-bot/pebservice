...
- **`client_config.secret.json`**: Generated by `setup_dev.py` from Secret Manager (needed only to run `get_token.py`'s OAuth flow).
- **`test-runner-key.secret.json`**: Generated by `setup_dev.py` — downloads the `peb-test-runner` SA key for running tests locally.

## IAM Verification & Troubleshooting
...
| Symptom | Cause | Fix |
|---|---|---|
| "Permission denied on secret" | SA lacks `secretmanager.secretAccessor` | Grant role on the specific secret |
| "Secret env var overlaps non-secret env var" | Mixing `--set-env-vars` and secret refs for same var | Delete function and redeploy, or update config |
| "Service account not found" | Wrong project selected | Verify with `gcloud config get-value project` |
| `process_email` stops processing | Pub/Sub agent lost `run.invoker` binding (see CAUTION above) | Re-add the binding on the `process-email` Cloud Run service |
